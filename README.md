...
| JD业务变量树选择 | STEP 3 按 A×L 加载有界子树，显示 canonical 追溯、配置侧、可见性、Hidden GT 和元数据 TBD |
...
3. **STEP 3**：检查 A×L 加载出的 JD业务变量树子树，勾选变量并生成选择清单 → 在清单范围内运行 Agent 提取；
4. **STEP 4**：编辑取值域；TBD 可按已知来源复核，来源不足时继续保留 TBD → 确认；
5. **STEP 5**：填写需要的案例数量（默认 10）→ 生成并审阅每个案例的完整 Task Template；需要复现时再展开高级 Seed；
6. **STEP 6**：分别检查世界侧与用户侧配置、Hidden GT、TBD 和校验结果 → 分别复制/下载 JSON，或下载完整交付包。
