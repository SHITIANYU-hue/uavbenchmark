...
STEP 3  JD V2 选变量    A×L 加载子树 → 人工勾选 → Agent 域提取
...
| 目标 Coverage | STEP 2 由人明确选择 A×L；当前无经确认的场景主测能力时不写死默认等级 |
| JD Version2 选择 | STEP 3 按 A×L 加载有界子树，显示 canonical 追溯、配置侧、可见性、Hidden GT 和元数据 TBD |
...
2. **STEP 2**：人工选择目标 A×L → 确认并扩充文案 → 确认文案并分类；
3. **STEP 3**：检查 A×L 加载出的 JD Version2 子树，勾选变量并生成选择清单 → 在清单范围内运行 Agent 提取；
...
|---|---|---|
| JD V2 变量选择 | `schemas/jd_tree_selection.schema.json` | 锁定本轮 V2 细粒度变量、canonical 追溯与 Agent 允许范围 |
...
pipeline/                     UI 模块：含 JD V2 子树选择、域编辑与 Seed 具体化
...
schemas/                      benchmark artifact schemas（含 JD V2 选择清单）
