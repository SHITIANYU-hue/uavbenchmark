...
把 17 个能力域从当前的跳跃式编号（A1, A2, A3, A4, **A14**, A5, A6a, A6b, A11, A7, A8, A9a, A9b, A10, A13, A12, A15）改成**连续的 A1–A17**。保留 `A` 前缀不变，去掉字母后缀（A6a/A6b、A9a/A9b）。同步重命名 JD 槽位 ID 让前缀对齐新编号。**G1–G6 编号不变**，但与 A 的关系变成连续区间（G1=A1–A5、G2=A6–A9、…、G6=A16–A17）。概念词 `A×L` / 「17 个 A」**保持不变**。
...
- A 顺序化后，G 组与 A 区间一一对应（见 §3.5），便于在代码里用区间查表替代显式字典。
...
| G 组编号 | **G1–G6 完全保留**，不改名；与 A 的关系变为连续区间（G1=A1–A5、…、G6=A16–A17） |
...
| A10  | **A14** | G5 | 通信 / C2 / 遥测 |
| A13  | **A15** | G5 | 空域 / 交通 / 合规接入 |
| A12  | **A16** | G6 | 安全包络 / 运行保证 / 处置建议 |
| A15  | **A17** | G6 | 日志 / 审计 / 保证证据 |

## 3.5 G 组（G1–G6）：编号不变，但与 A 的关系变成连续区间

**G1–G6 的编号与命名完全不变**，依然是 6 个组。但由于 A 已顺序化为 A1–A17，每个 G 现在对应一个**连续的 A 区间**：
...
| G5 | A14–A15 | 2 | 通信 / 空域合规 |
| G6 | A16–A17 | 2 | 安全包络 / 日志审计 |

带来的简化：

1. **`src/uav_benchmark/compiler/intake.py` 的 `ABILITY_TO_GROUP` 可以从 17 项字典简化为区间查表**（可选优化；保留显式字典也可读性更好，二选一）：
...
2. **`pipeline/js/jd.js` 的 `groupCoverageByG` 不需要改**：它通过 `abilityMeta(a_id).g_group` 动态查表，本来就是数据驱动。
3. **docs / UI 文案中的 `G1`–`G6` 引用全部保留**，无需替换。
...
| jd-12.N  | A12  | **jd-16.N**  | A16 | ⚠ ID 交换 |
| jd-13.N  | A13  | **jd-15.N**  | A15 | ⚠ ID 交换 |
| jd-14.N  | A14  | **jd-5.N**   | A5  | ⚠ ID 交换 |
| jd-15.N  | A15  | **jd-17.N**  | A17 | — |
...
- 旧 `jd-12.N`（A12）、旧 `jd-13.N`（A13）、旧 `jd-15.N`（A15）也需要协同改
...
6. **改代码与 schema**：`intake.py`（17 个键，或可选改成 §3.5 的区间查表）、两个 schema enum、`jd.js` 两处正则、`constants.js` 键名。
...
- [ ] §3 的 A 编号映射是否最终采纳？（特别是 A14→A5、A12→A16、A15→A17 这类换号）
- [ ] §3.5 的 `ABILITY_TO_GROUP` 是保留 17 项显式字典，还是改成区间查表？
