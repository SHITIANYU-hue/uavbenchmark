...
1. 页面按 STEP 2 的 A×L 只加载对应能力变量，不显示全局变量，也不展开完整 444 节点
2. 默认按等级累计勾选：L2 包含 L1–L2，L3 包含 L1–L3，L4 包含 L1–L4
3. 检查每个变量的 canonical JD、配置侧、role、visibility、Hidden GT 和 TBD
4. 调整勾选；“恢复 L 累计建议”只恢复建议范围，不代表业务批准
5. **确认选择并生成清单**；可复制或下载 `jd_tree_selection.json`
6. **运行 JD 域提取**；Agent 只能使用清单内映射出的 canonical JD
...
3. 用案例标签切换查看；重点审阅两段文字：
   - **审阅正文**：使用 `【canonical JD＝本案例实际取值】` 标注变量。原文已出现
     的值直接在原句位置标注，其余变量写入七段骨架中对应的语义段落；第七段后不再
     追加 JD 清单
   - **SUT 可见正文**：去掉 Hidden GT 和世界侧私有信息，可直接交给任务接口
4. 查看默认展开的“本案例具体化明细”，核对 Seed、A×L、STEP 4 取值域、本案例
   实际值、配置归属和运行计划
...
6. 如果前面步骤修改过，点顶部 **按复核结果重新生成**，再导出最新产物
...
所有步骤的主操作均集中在页面顶部“本步操作”。STEP 1 还提供“一键生成草案到
STEP 6”；它用于快速得到一版可审阅草案，不代替中间步骤的人工确认。
