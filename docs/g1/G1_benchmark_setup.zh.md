...
> 编号基线：本文已迁移到 `ability-id-v3-2026-07-20`；G1 监督 / 请求 / 交接能力使用 A5。
>
...
>
> **术语（三分，勿混）**：**用户模**（user model / `user_model` = `execution_anchors ⊕ persona`，对用户建模；旧称"用户情境"，替换 `user_membrane`）/ **交互膜**（A1/A5 conversation 面，"膜"只指这个）/ **events**（collapse 出的轮次）。链条：用户模 →(materializer 在)→ 交互膜 →(collapse)→ events →(judge 投影 anchors)→ GT。
...
G1 = 人机交互组 = **A1 / A2 / A3 / A4 / A5**（Avant-Bench 流水线图的五条边）：

| A | 边 | 归属 | 我们做什么 |
|---|---|---|---|
| **A1** | 用户→harness（下达） | 用户膜（域不变） | **用户侧出招** |
| **A5** | harness→用户（请示/汇报/回显） | 用户膜（域不变） | **用户侧收招 + 判分主来源** |
| A2/A3/A4 | 操作接口/loop（SUT 内环） | SUT 内部 | 不碰，只从 trace 观测 |

- **A1/A5 域不变**：D-001/D-002/C-001 用同一套 schema，只换 world + 业务知识。
- **G1-ness = (A1 或 A5 非 L0)**：A1×L0（非对话发起，种子从 A2/世界事件进）+ A5≥L1（agent 单向汇报）= **监督型 G1**；只有 A1×L0 ∧ A5×L0 才出 G1。
...
execution_anchors:                # "一条正确运行必须办成/说清什么";judge 时逐条投影→GT
  ...
  required_confirmations: [ {echo_has, wait}, ... ] # A5×L3 → agent 须回显+等确认
...
- **persona 库 = 用户侧的 worlds.库**；两条正交子轴：**persona-style**（爱聊/话少/多疑/急…压 A3/A5）× **interaction-pattern**（配合/含糊/变卦…压特定格）。
...
loop:
   SUT 处理 → 落 A5(请示/回显/汇报) 或 动作 → 进入 等
   agent 一「等」→ 用户侧跑 上下文框架下想(persona框架) → 产下一句 A1 → 喂回 SUT「使用」
until  goal_predicate 满足 | 预算耗尽(轮数上限 = 1 + 澄清(A1_L) + 汇报/确认(A5_L))
```
...
交互点 = anchors 字段（§5 principles）：意图/动作/终态（沿用 pre/actions/post）+ 澄清/披露/确认/兜底/达成（新增，挂 A1/A5）。
...
| A3 | 后续持续步 | `继续`/`整合` | 域不变 |
| A5 | 发消息/留存 | `发`/`记下`(留存) | 域不变 |
| A4 | 对实体的编排 | 域动词(drone_command / 改文件·跑命令) | **分域** |

**A1/A2/A3/A5 踩 meta-V（域不变）、A4 踩 domain-V（分域）**——用户侧域不变、A4 是 G1→G2/G3 的桥。黑盒 SUT 下 A2/A3 只能踩效果/reasoning（U-Q7）。
