...
src/uav_benchmark/agent/      DeepSeek Config Agent 与本地 HTTP 服务
...
| 接口 | 作用 |
|---|---|
| `GET /api/health` | 本地 Agent / DeepSeek 配置状态 |
...
- 组织 DeepSeek Structured Output 请求（文案 + coverage/extraction）；
- 把模型响应转换为 Pydantic 合同；
- 执行确定性后校验。

第一次请求由 `NarrativeAgent` 执行，只输出业务文案。第二次由 `ConfigAgent` 分类和提取；服务端用人工确认稿覆盖模型响应中的正文。
